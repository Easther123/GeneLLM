...
- Provide evidence and reasoning for each inferred GO term, referring to insights derived from KEGG pathways, homologous gene functions, co-expression networks, and TWAS results.
...
6. **KEGG Pathway Data** (High Confidence):
...
1. **Priority Weighting**:
   - Give precedence to higher-confidence sources in the following order:  
     - TWAS > Homolog > KEGG > Expression > Upregulation/Downregulation Analysis > BLAST > Co-expression.
   - Prioritize data that has strong statistical or experimental support (e.g., TWAS Z-scores, peer-reviewed functional studies, or well-curated KEGG pathways).
...
2. **Consensus Building**:
   ...
   - Integrate data that aligns across datasets (e.g., TWAS, homologs, and KEGG pathways) and explicitly state areas of agreement.
...
4. **External Validation**:
   - Use external knowledge sources (e.g., STRING, PlantTFDB) to resolve ambiguities and validate the most plausible prediction. Avoid relying on external GO annotations, as local data provides sufficient evidence for inferred GO terms.
...
- Use KEGG data to identify biological processes, molecular functions, cellular components, and pathways related to the gene. Highlight how these annotations support predictions derived from other datasets (e.g., TWAS, homologs, expression).
- Infer potential GO terms based on other data (e.g., KEGG, homologs, co-expression, TWAS). Correlate these inferred GO terms with gene expression trends to strengthen hypotheses about the gene's role in traits or functions.
...
- For each section (e.g., predicted function, traits, evidence), include:
  ...
  3. Integration of KEGG annotations with other datasets to build a robust hypothesis.
...
1. **Predicted Function and Traits**:
   ...
   - Highlight the roles suggested by KEGG data, explicitly linking them to the predicted traits or functions.
...
2. **Explanation of Reasoning**:
   ...
   - Emphasize how KEGG annotations complement findings from TWAS, homologous genes, and expression data.
...
3. **Upregulation/Downregulation Predictions**:
   ...
   - Analyze whether gene '{gene_id}' is likely to be upregulated or downregulated under specific conditions or tissues based on:
     ...
     2. KEGG pathway roles that may highlight regulatory or metabolic functions.
   - For each trend, provide:
     - A clear statement of whether the gene is upregulated or downregulated.
     - Supporting evidence from the provided KEGG annotations.
...
4. **Integration of KEGG Data**:
   - Summarize how KEGG annotations support or refine the predictions for gene '{gene_id}'.
...
5. **External Validation**:
   - Supplement predictions with insights from external knowledge sources, excluding GO data. These sources may include:
   ...
   - For each GO term, provide:
     ...
     2. Supporting evidence from datasets (e.g., KEGG pathways, homolog data, co-expression analysis).
...
#### **Task Steps**:
To validate and integrate all evidence, including KEGG data, follow these steps in order:
...
4. **Integrate Local Data (Including KEGG)**:
   - Combine KEGG data with other local evidence (e.g., TWAS, expression, homologs) to reinforce predictions.
   - Explicitly state whether the data supports or contradicts the hypothesis:
     - If KEGG pathways suggest metabolic roles aligned with expression data or TWAS findings, use this alignment to strengthen predictions.
     - If KEGG pathways suggest roles that contradict homolog functions, evaluate pathway relevance and propose possible explanations.

5. **Resolve Conflicts Between Evidence**:
   - If data from different sources (e.g., KEGG vs. expression) presents conflicting predictions:
...
6. **Prioritize Biological Plausibility**:
   - Assign higher priority to data that aligns with:
     - Known biological mechanisms (e.g., stress response pathways in KEGG).
     - Statistical robustness (e.g., TWAS Z-scores or high-confidence protein-protein interactions).
     - Peer-reviewed annotations in KEGG or literature.

 7. **Infer Potential GO Terms**:
   - Use integrated evidence from KEGG, homologs, TWAS, and expression data to predict potential GO terms.
...
#### **Expected Results**:
Summarize the insights derived from all datasets, including KEGG annotations, and explain how they integrate with other evidence. Example outputs include:
...
Clearly indicate the alignment or conflict between KEGG annotations and other evidence (e.g., expression, TWAS) to provide a robust final prediction.
...
- Indicate the confidence level of the prediction based on the consistency and strength of the evidence across datasets, including:
  - **High Confidence**: When KEGG pathways and other local evidence (e.g., TWAS, expression) align to support a consistent prediction.
...
Gene '{gene_id}' is predicted to be involved in **drought stress response** and **fiber development**, functioning as a **negative regulator** in stress-mediated signaling pathways. This prediction is supported by multiple lines of evidence, including homologous gene data, tissue-specific expression patterns, co-expression networks, TWAS associations, and local annotations from KEGG pathways.
---
### **Supporting Evidence**:
...
Therefore, the expression data confirms the gene's dual role in fiber development and abiotic stress tolerance.
---
...
---
#### **6. KEGG Annotations**:
Local KEGG annotations further strengthen the prediction for '{gene_id}':
- **KEGG Pathways**:
...
---
### **Conclusion**:
In summary, integrating evidence from homologous gene data, expression patterns, co-expression networks, TWAS associations, BLAST similarity, and local annotations from KEGG, '{gene_id}' is predicted to function as a **key regulator in drought tolerance and fiber development**. Its **upregulation** in fiber and ovule tissues, combined with supporting data from KEGG annotations, confirms its critical role in cotton's agronomic traits and stress adaptation mechanisms.
### **Example Citations**:
...
def save_prompt_to_file(gene_id, file_path):
    # 假设这个函数是用来保存某些数据到文件的
    # 这里只是一个占位符，你需要根据实际情况实现这个函数
    with open(file_path, 'w') as file:
        file.write(f"Data for gene ID: {gene_id}\n")
    print(f"Processed gene: {gene_id}")

if __name__ == "__main__":
    # 定义一个包含所有基因ID的列表
    gene_ids = [
        "Ghir_A09G011090", "Ghir_D08G012390", "Ghir_A12G026360", "Ghir_D12G026390",
        "Ghir_A12G024190", "Ghir_D01G023410", "Ghir_A06G005490", "Ghir_D04G013830",
        "Ghir_A04G009600", "Ghir_D13G002750", "Ghir_A12G016570", "Ghir_A13G002460",
        "Ghir_A01G021910", "Ghir_D02G005050", "Ghir_A05G003820", "Ghir_D09G023920",
        "Ghir_D09G006620", "Ghir_D07G012650", "Ghir_A09G024770", "Ghir_D05G003930",
        "Ghir_D13G003600", "Ghir_D01G003630", "Ghir_D05G034270", "Ghir_A11G025790",
        "Ghir_A02G004730", "Ghir_A10G003550", "Ghir_A13G010580", "Ghir_A09G006910",
        "Ghir_D13G011170", "Ghir_A04G004940", "Ghir_A08G023990", "Ghir_A01G003600",
        "Ghir_A06G008940", "Ghir_D08G023570", "Ghir_A13G003340", "Ghir_D03G003270",
        "Ghir_A05G036750", "Ghir_D01G005060", "Ghir_A09G003360", "Ghir_D01G007970",
        "Ghir_A02G004820", "Ghir_A01G007620", "Ghir_A11G008740", "Ghir_D01G018720",
        "Ghir_A09G025510", "Ghir_D02G005130", "Ghir_A10G017640", "Ghir_D10G017900",
        "Ghir_A10G016420", "Ghir_A04G003900"
    ]

    # 定义基础路径
    base_path = r"C:\Users\29901\PycharmProjects\PythonProject2\mianhua_data"

    # 遍历基因ID列表并为每个基因调用函数
    for gene_id in gene_ids:
        # 使用os.path.join来确保跨平台兼容性
        file_path = os.path.join(base_path, f"{gene_id}_data.txt")
